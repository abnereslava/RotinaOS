...
function clearUnprogrammedPressState() {
  const button = document.getElementById('calendar-unprogrammed');
  if (!button) return;
...
function bindPressFeedback(button) {
  if (!button || button.dataset.pressBound === 'true') return;
  button.dataset.pressBound = 'true';

  const clear = () => button.classList.remove('is-pressing');
...
function createCalendarScreen() {
  ...
  screen.innerHTML = `
    ...
    <div class="calendar-scroll-area">
      ...
      <button id="calendar-new-activity-day" class="btn-primary calendar-new-day-activity" type="button">
        <i class="fas fa-plus"></i>
        <span>NOVA ATIVIDADE NESTE DIA</span>
      </button>
  ...
  document.getElementById('calendar-new-activity-day')?.addEventListener('click', () => {
    if (selectedDate) openNewActivityForDate(selectedDate);
  });

  const unprogrammedButton = document.getElementById('calendar-unprogrammed');
  bindPressFeedback(unprogrammedButton);
  unprogrammedButton?.addEventListener('click', event => {
    event.currentTarget.classList.remove('is-pressing');
    event.currentTarget.blur();
    openActivityBankFromCalendar();
  });
...
function createMonthPicker() {
  ...
  overlay.addEventListener('click', event => {
    if (event.target === overlay || event.target.closest('[data-close-calendar-control]')) {
      closeControlModal(overlay);
    }
  });
...
function renderCategoryOptions() {
  ...
  categories.forEach(category => {
    ...
    input.addEventListener('change', () => {
      ...
      if (
        categoryFilter.size === allCategories.length &&
        allCategories.every(item => categoryFilter.has(item))
      ) {
        categoryFilter = null;
      }
...
function renderCalendar() {
  ...
  if (horizon) {
    horizon.textContent = `de ${formatMonthTitle(windowInfo.minYear, windowInfo.minMonth)} a ${formatMonthTitle(windowInfo.maxYear, windowInfo.maxMonth)}`;
  }
  ...
  renderNewActivityButton(windowInfo);
  renderSelectedDay();
...
function renderMonthGrid(windowInfo) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    // Tocar no dia apenas muda o destaque e mostra as tarefas daquele dia.
...
function renderNewActivityButton(windowInfo) {
  const button = document.getElementById('calendar-new-activity-day');
  if (!button || !selectedDate) return;

  const isPast = selectedDate < windowInfo.todayKey;
  const label = button.querySelector('span');

  button.disabled = isPast;
  button.title = isPast
    ? 'Datas anteriores são apenas para consulta.'
    : `Criar atividade programada para ${formatShortDate(selectedDate)}`;

  if (label) {
    label.textContent = isPast
      ? 'DATA ANTERIOR — SOMENTE CONSULTA'
      : `NOVA ATIVIDADE EM ${formatShortDate(selectedDate)}`;
  }
}
